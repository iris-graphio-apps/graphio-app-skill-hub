...
description: graphio app 개발(앱·노드·테스트·온톨로지 등) 중 필요한 사내 스킬·역량이 로컬에 없으면 반드시 이 스킬을 먼저 쓴다. graphio-app-skill-hub 레지스트리(registry.json)를 읽어 사용자의 need 를 각 스킬 description 과 매칭하고, shallow+sparse git clone 으로 해당 skills/이름 폴더를 현재 도구의 스킬 디렉터리(.claude/skills, --user 시 ~/.claude/skills)에 설치·갱신한다. Claude 가 맥락으로 자동 호출하거나, 사용자가 /mobigen-skill-installer 로 직접 부를 수 있다(뒤에 스킬 이름을 주면 바로 설치, list 면 목록). '스킬 설치', '스킬 찾아줘', '스킬 받아', 'skillhub', '레지스트리에서', '사내 스킬 없네', 'graphio 스킬 깔아', 'graphio 앱 만들자', 'skill install', 'skill update', '필요한 스킬 가져와' 같은 표현·맥락에서 사용. 단, 새 스킬을 만들어 hub 에 올리는 작업은 mobigen-skill-creator 를, 이미 설치된 스킬의 기능 자체는 그 스킬을 쓴다. 모든 설명은 한국어로, 코드/식별자는 영어 그대로.
---
...
기본값(`disable-model-invocation` 미설정)이라 **모델 자동 호출**과 **사용자 직접 호출**이 모두 된다.

- **자동(모델 판단)**: graphio 작업 중 필요한 역량이 로컬에 없다고 Claude 가 판단하면 이 스킬을 consult 한다. 단 자동은 **보장이 아니다**(Claude 가 스킬을 과소트리거할 수 있음) — 그래서 아래 직접 호출을 **폴백**으로 둔다.
- **직접 호출(폴백, 권장)** — 사용자가 슬래시로:
  - `/mobigen-skill-installer` — need 를 묻거나 목록을 보여주고 고르게 한다
  - `/mobigen-skill-installer <스킬이름>` — 그 스킬을 바로 설치 (예: `/mobigen-skill-installer graphio-app-test`)
  - `/mobigen-skill-installer list` — 레지스트리 목록/검색
- **스크립트 직접** — 언제든 `python3 scripts/skillhub.py list|install|update ...`

호출 인자(arguments)가 들어오면: 스킬 이름이면 그걸 install, `list`/`search` 면 목록, 비어 있으면 아래 §0 흐름(need 매칭).
...
4. **설치한다** — `skillhub.py install <name>` 으로 설치하고 위치를 알려준다. 이어서 같은 턴에 쓰려면 §2 의 반영 규칙을 따른다.

이름을 명시(직접 호출 인자 포함)하면 1~3을 건너뛰고 바로 4로 설치한다.
...
- 설치 위치가 **세션 시작 시 이미 존재하던** `.claude/skills/`(프로젝트 또는 사용자)면 **재시작 없이 현재 세션에 자동 반영**된다.
- **같은 턴에 바로 쓰려면** 방금 설치한 `<dst>/SKILL.md` 를 **Read 해서 그 지침을 따르면 된다** — available-skills 목록 갱신을 기다릴 필요가 없다.
- 단, **세션 시작 시 없던 디렉터리**(예: 첫 `~/.claude/skills/`)에 처음 설치하면 그 디렉터리를 감시하려고 **재시작이 필요**할 수 있다 → 그 경우 위의 Read-후-따르기로 우회한다.
...
| 설치했는데 자동 인식 안 됨 | 기존 `.claude/skills/` 면 세션 내 반영. 세션 시작 시 없던 새 디렉터리면 재시작 필요 → 같은 턴엔 방금 설치한 `SKILL.md` 를 Read 해 따른다. 경로가 `.claude/skills/<name>/SKILL.md` 인지 확인 |
| 자동 트리거가 안 됨 | 정상적 한계(모델 판단). `/mobigen-skill-installer <스킬이름>` 으로 직접 호출 |
